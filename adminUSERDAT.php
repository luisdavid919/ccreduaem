...
switch ($accion) {
	case 'btnAgregar':
			...
			if ($enroll=="") {
				$error['enroll']="Por Favor, Escriba Su Matrícula";
			}
		...
		$sentencia=$pdo->prepare("INSERT INTO estudiante(name,lastname,age,profession,enroll,period,sem,turn,img) VALUES (:name,:lastname,:age,:profession,:enroll,:period,:sem,:turn,:img)");
	...
	case 'btnEditar':
		$sentencia=$pdo->prepare("UPDATE estudiante SET
			name=:name,
			lastname=:lastname,
			age=:age,
		...
		$sentencia=$pdo->prepare("SELECT img FROM estudiante WHERE id=:id");
		$sentencia->bindParam(':id',$id);
		$sentencia->execute();
	...
	case 'btnEliminar':
		...
		$sentencia=$pdo->prepare("DELETE FROM estudiante WHERE id=:id");
		$sentencia->bindParam(':id',$id);
		$sentencia->execute();
	...
	case 'Editar':
		...
		$sentencia=$pdo->prepare("SELECT * FROM estudiante WHERE id=:id");
		$sentencia->bindParam(':id',$id);
		$sentencia->execute();
		$PC=$sentencia->fetch(PDO::FETCH_LAZY);
		...
		$sentencia= $pdo->prepare("SELECT * FROM estudiante WHERE 1");
		$sentencia->execute();
  	...
  	<div class="row">
  		<div class="col">
			...
			<button type="button" class="btn btn-success" data-toggle="modal" data-target="#exampleModal"><i class="fas fa-plus-square"></i> Agregar Datos </button>
  			<a href="http://localhost/ccreduaem/adminUSERDAT.php" class="btn btn-info m-1" role="button"><i class="fas fa-users"></i> Ver Aministradores</a>
 		</div>
  	</div>
  </div>
 			 ...
 			 <div class="modal-dialog modal-lg" role="document">
    			<div class="modal-content">
      			<div class="modal-header">
        			<h5 class="modal-title" id="exampleModalLabel">Datos Estudiantes</h5>
      			...
      			<div class="modal-body">
        			<div class="form-row">
						...
						<div class="form-group col-lg-4">
							...
							<select class="form-control" name="sem" id="sem" value="<?php echo $sem;?>">
...
<!--*****************Regresar*****************-->
			<div class="container-fluid fixed-bottom">
			  <div class="row m-5">
			    <div class="col">
			      <a href="http://localhost/ccreduaem/adminDAT.php" data-toggle="tooltip" data-placement="right" title="Regresar"><i class="fas fa-chevron-left fa-2x" style="color:black;"></i></a>
			    </div>
			  </div>
			</div>
