...
switch ($accion) {
	case 'btnAgregar':
		...
		$sentencia=$pdo->prepare("INSERT INTO mouse(serials,marc,model,estado,img) VALUES (:serials,:marc,:model,:estado,:img)");
	...
	case 'btnEditar':
		$sentencia=$pdo->prepare("UPDATE mouse SET
			serials=:serials,
			marc=:marc,
			model=:model,
		...
		$sentencia=$pdo->prepare("SELECT img FROM mouse WHERE id=:id");
		$sentencia->bindParam(':id',$id);
		$sentencia->execute();
	...
	case 'btnEliminar':
		...
		$sentencia=$pdo->prepare("DELETE FROM mouse WHERE id=:id");
		$sentencia->bindParam(':id',$id);
		$sentencia->execute();
	...
	case 'Editar':
		...
		$sentencia=$pdo->prepare("SELECT * FROM mouse WHERE id=:id");
		$sentencia->bindParam(':id',$id);
		$sentencia->execute();
		$PC=$sentencia->fetch(PDO::FETCH_LAZY);
