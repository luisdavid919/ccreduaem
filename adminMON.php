...
switch ($accion) {
	case 'btnAgregar':
		...
		$sentencia=$pdo->prepare("INSERT INTO monitor(serials,marc,model,estado,img) VALUES (:serials,:marc,:model,:estado,:img)");
		...
		echo       
             '<META HTTP-EQUIV="Refresh" CONTENT="0; URL=http://localhost/ccreduaem/adminMON.php">
	...
	case 'btnEditar':
		$sentencia=$pdo->prepare("UPDATE monitor SET
			serials=:serials,
			marc=:marc,
			model=:model,
		...
		$sentencia=$pdo->prepare("SELECT img FROM monitor WHERE id=:id");
		$sentencia->bindParam(':id',$id);
		$sentencia->execute();
	...
	case 'btnEliminar':
		...
		$sentencia=$pdo->prepare("DELETE FROM monitor WHERE id=:id");
		$sentencia->bindParam(':id',$id);
		$sentencia->execute();
	...
	case 'Editar':
		...
		$sentencia=$pdo->prepare("SELECT * FROM monitor WHERE id=:id");
		$sentencia->bindParam(':id',$id);
		$sentencia->execute();
		$PC=$sentencia->fetch(PDO::FETCH_LAZY);
		...
		$sentencia= $pdo->prepare("SELECT * FROM monitor WHERE 1");
		$sentencia->execute();
